...
package net.kdt.pojavlaunch.mobileglues.ui.material
...
import net.kdt.pojavlaunch.mobileglues.settings.SpinnerOption
import net.kdt.pojavlaunch.mobileglues.ui.AppController
import net.kdt.pojavlaunch.mobileglues.ui.SettingsLoadState

/**
 * 设置页。
 *
 * MG 目录是 App 私有存储（Tools.DIR_DATA/MobileGlues），读写 config.json 不需要任何存储权限，
 * 所以这里不设权限门，加载完成即可直接展示渲染器配置。
 */
@Composable
fun MaterialSettingsPage(controller: AppController) {
    ...
    Column(
        ...
        Crossfade(
            targetState = loadState == SettingsLoadState.Ready && config != null,
            label = "settings-gate",
        ) { ready ->
            Column(modifier = Modifier.fillMaxWidth()) {
                if (ready) {
                    ConfigSections(controller, config ?: MGConfig.Default)
                } else {
                    CenteredLoading(modifier = Modifier.padding(top = 48.dp))
                }
            }
        }
